  ...
  return (
    <section id="experience" className="py-20 bg-slate-950 relative">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute bottom-20 left-10 w-96 h-96 bg-sky-500/5 rounded-full blur-3xl"></div>
      </div>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="space-y-12">
          {education.length > 0 && (
            <div>
              <h3 className="text-2xl font-bold text-sky-400 mb-8 flex items-center gap-3 animate-fade-up">
                <span className="w-1 h-8 bg-gradient-to-b from-sky-400 to-transparent rounded"></span>
                Education
              </h3>
              <div className="space-y-6">
                {education.map((item, index) => (
                  <div key={item.id} className="relative pl-8 animate-fade-up" style={{ animationDelay: `${index * 0.1}s` }}>
                    <div className="absolute left-0 top-0 w-4 h-4 rounded-full bg-sky-500 border-2 border-slate-950 animate-pulse-glow"></div>
                    <div className="p-6 rounded-lg bg-gradient-to-br from-sky-950/20 to-slate-950 border border-sky-500/30 hover:border-sky-500/50 transition-all duration-300">
                      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-2 mb-3">
                        <div>
                          <h4 className="text-xl font-bold text-white">{item.title}</h4>
                          <p className="text-sky-400 font-medium">{item.organization}</p>
                        </div>
                      ...
                      {item.location && (<p className="text-slate-400 text-sm mb-3 flex items-center gap-2"><MapPin size={16} />{item.location}</p>)}
