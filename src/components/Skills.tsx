...
export default function Skills() {
  ...
  const getLevelColor = (level: string) => {
    switch (level) {
      case 'beginner': return 'from-sky-600 to-cyan-700';
      case 'intermediate': return 'from-sky-500 to-sky-600';
      case 'advanced': return 'from-cyan-400 to-sky-500';
  ...
  return (
    <section id="skills" className="py-20 bg-gradient-to-b from-slate-950 to-sky-950/5 relative">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/2 right-0 w-96 h-96 bg-sky-500/5 rounded-full blur-3xl"></div>
      </div>
            ...
            {Object.entries(groupedSkills).map(([category, categorySkills], categoryIndex) => (
              <div key={category} className="space-y-6 animate-fade-up" style={{ animationDelay: `${categoryIndex * 0.2}s` }}>
                <h3 className="text-2xl font-bold text-sky-400 flex items-center gap-3">
                  <span className="w-8 h-8 rounded-full bg-sky-500/20 border border-sky-500/50 flex items-center justify-center text-sm">{categorySkills.length}</span>
                  {category}
                </h3>
                  ...
                  {categorySkills.map((skill, skillIndex) => (
                    <div key={skill.id}>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-slate-300 font-medium group-hover:text-sky-400 transition-colors">{skill.name}</span>
                        <span className="text-xs text-slate-500 uppercase tracking-wider">{skill.level}</span>
                      </div>
                      <div className="h-2 bg-sky-950/40 rounded-full overflow-hidden border border-sky-500/20">
